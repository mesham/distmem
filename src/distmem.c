...
void init_distmem() {
	dist_memory_kinds=(struct dist_intenal_distmem_node**) memkind_malloc(MEMKIND_DEFAULT, sizeof(struct dist_intenal_distmem_node*) * HASH_SIZE);
	int i;
...
int distmem_arena_create(struct distmem * dist_kind, struct distmem_ops *ops, const char *name) {
	...
	dist_kind->internal_state=
			(struct distmem_memory_information_generic**) memkind_malloc(MEMKIND_DEFAULT, sizeof(struct distmem_memory_information_generic*) * HASH_SIZE);
	for (i=0;i<HASH_SIZE;i++) {
...
void distmem_put_specific_entry_into_state(struct distmem * dist_kind, void * specific_info, void * ptr) {
	...
	struct distmem_memory_information_generic* generic_info=(struct distmem_memory_information_generic*) memkind_malloc(MEMKIND_DEFAULT, sizeof(struct distmem_memory_information_generic));
...
static void put_kind_entry_into_kind_state(struct distmem * dist_kind) {
	...
	struct dist_intenal_distmem_node* specific_node=(struct dist_intenal_distmem_node*) memkind_malloc(MEMKIND_DEFAULT, sizeof(struct dist_intenal_distmem_node));
...
static int get_hashkey(void * buffer) {
	return ((uintptr_t) buffer) % HASH_SIZE;
}
