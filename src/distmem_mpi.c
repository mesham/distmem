...
void* distmem_mpi_arena_malloc(struct distmem* dist_kind, size_t element_size, size_t number_elements, int nargs, ...) {
  ...
  for (i = 0; i < size; i++) {
    elements_per_proc[i] = dimension_division + (i < dimension_extra ? 1 : 0);
  }
  put_info_entry_into_state(dist_kind, buffer, size, element_size, number_elements, number_local_elements, elements_per_proc,
                            communicator);
  return buffer;
}
