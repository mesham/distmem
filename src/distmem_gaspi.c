...
static struct distmem_ops GASPI_DISTRIBUTED_MEMORY_VTABLE = {
    .dist_malloc = distmem_gaspi_arena_malloc,
...
static void put_info_entry_into_state(struct distmem *, void *, int, size_t,
                                      size_t, size_t);

void distmem_gaspi_init() {
  init_distmem();
  int err = distmem_create_default(&GASPI_DISTRIBUTED_MEMORY_VTABLE,
                                   "gaspi contiguous", &GASPI_CONTIGUOUS_KIND);
  if (err) {
...
void distmem_gaspi_create_segment_kind(const char *name, memkind_t *kind,
...
void *distmem_gaspi_malloc(memkind_t kind, size_t element_size,
                           size_t number_elements, gaspi_group_t gaspi_group) {
  struct distmem *dist_kind = distmem_get_distkind_by_name(kind->name);
  return dist_kind->operations->dist_malloc(dist_kind, element_size,
                                            number_elements, 1, gaspi_group);
}

void *distmem_gaspi_arena_malloc(struct distmem *dist_kind, size_t element_size,
                                 size_t number_elements, int nargs, ...) {
  va_list ap;
  va_start(ap, nargs);
  gaspi_group_t gaspi_group = (gaspi_group_t)va_arg(
      ap, int);  // As always promoted to int when passed through vargs
  ...
  number_local_elements =
      dimension_division + (my_rank < dimension_extra ? 1 : 0);
  void *buffer = (char *)memkind_malloc(dist_kind->memkind,
                                        number_local_elements * element_size);
  put_info_entry_into_state(dist_kind, buffer, size, element_size,
                            number_elements, number_local_elements);
  return buffer;
}

struct distmem_gaspi_memory_information *distmem_gaspi_get_info(memkind_t kind,
                                                                void *ptr) {
  struct distmem *dist_kind = distmem_get_distkind_by_name(kind->name);
  return (struct distmem_gaspi_memory_information *)distmem_get_specific_entry(
      dist_kind, ptr);
}

static void put_info_entry_into_state(struct distmem *dist_kind,
                                      void *local_buffer,
                                      int procs_distributed_over,
                                      size_t element_size,
                                      size_t total_number_elements,
                                      size_t number_local_elements) {
  struct distmem_gaspi_memory_information *specific_state =
      (struct distmem_gaspi_memory_information *)memkind_malloc(
          MEMKIND_DEFAULT, sizeof(struct distmem_gaspi_memory_information));
  ...
  distmem_put_specific_entry_into_state(dist_kind, specific_state,
                                        local_buffer);
}
